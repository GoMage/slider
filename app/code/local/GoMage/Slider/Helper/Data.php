...
class GoMage_Slider_Helper_Data extends Mage_Core_Helper_Abstract{
	...
	public function a($k, $c = 0, $s = ''){
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, 'key='.urlencode($k).'&sku=slider_pro&domains='.urlencode(implode(',', $this->getAllStoreDomains())));
        ...
        if (!$s) $s = Mage::getStoreConfig('gomage_activation/slider/websites');
    ...
    {    
        ...
        return array(
        	GoMage_Slider_Model_Item::EFFECT_SIMPLE => $helper->__('Simple'),
            GoMage_Slider_Model_Item::EFFECT_VERTICAL_SPLIT => $helper->__('Vertical split'),
            GoMage_Slider_Model_Item::EFFECT_HORIZONTAL_SPLIT => $helper->__('Horizontal split'),
            GoMage_Slider_Model_Item::EFFECT_WIPE_RIGHT => $helper->__('Wipe right'),
            GoMage_Slider_Model_Item::EFFECT_WIPE_LEFT => $helper->__('Wipe left'),
            GoMage_Slider_Model_Item::EFFECT_WIPE_UP => $helper->__('Wipe up'),
            GoMage_Slider_Model_Item::EFFECT_WIPE_DOWN => $helper->__('Wipe down'),
            GoMage_Slider_Model_Item::EFFECT_PAGE_FLIP => $helper->__('Page Flip'),
            GoMage_Slider_Model_Item::EFFECT_HORIZONTAL_PANELS => $helper->__('Horizontal panels'),
            GoMage_Slider_Model_Item::EFFECT_VERTICAL_PANELS => $helper->__('Vertical panels')
        );
    }
	
	public function getSlideTextStyle($item, $block)
    {
    	$style = '';
    	
    	if ( $item->getTextWindowLeftIndent() != '' )
    	{
    		$style .= 'margin-left: ' . (int)$item->getTextWindowLeftIndent() . 'px;';	
    		$leftIndent = (int)$item->getTextWindowLeftIndent();
    	}
    	
    	if ( $item->getTextWindowTopIndent() != '' )
    	{
    		$style .= 'margin-top: ' . (int)$item->getTextWindowTopIndent() . 'px;';
    		$topIndent = (int)$item->getTextWindowTopIndent();	
    	}
    	
    	if ( $item->getTextWindowWidth() != '' && (int)$item->getTextWindowWidth() > 0 )
    	{
    		$style .= 'width: ' . (int)$item->getTextWindowWidth() . 'px;';
    	}
    	else 
    	{
    	...
    	if ( $item->getTextWindowHeight() != '' && (int)$item->getTextWindowHeight() > 0 )
    	{
    		$style .= 'height: ' . (int)$item->getTextWindowHeight() . 'px;';
    	}	
    	else 
    	{
    		if ( $block->getShowNavigationBar() == GoMage_Slider_Model_Adminhtml_System_Config_Source_Navigation_Show::SIDEBAR )
    	...
    	if ( $item->getTextWindowAlignment() == GoMage_Slider_Model_Adminhtml_System_Config_Source_Slide_Windowalignment::RIGHT )
    	{
	...
	public function getSlideTextBgStyle($item, $block)
    {
    	$style = '';
    	...
    	if ( $item->getTextWindowBackground() != '' )
    	{
    		$style .= 'background-color: ' . $item->getTextWindowBackground() . ';';
	    	...
	    	if ( $item->getBackgroundOpacity() != '' )
	    	{
	    		...
	    		if ( $this->_detect_ie() )
	    		{
	    			...
	    			if ( $item->getBackgroundOpacity() > 0 && $item->getBackgroundOpacity() < 1 )
	    			{
	    				$opacity = $item->getBackgroundOpacity() * 100;
	    			}
	    		...
	    		{
	    			$style .= 'opacity: ' . $item->getBackgroundOpacity() . ';';
	    		}	
	    	}
    	}
    ...
    public function notify(){
        ...
        try{
            ...
            curl_setopt($ch, CURLOPT_POST, true);
            curl_setopt($ch, CURLOPT_POSTFIELDS, 'sku=sliser&timestamp='.$timestamp.'&ver='.urlencode('1.1'));
            curl_setopt($ch, CURLOPT_TIMEOUT, 30);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
