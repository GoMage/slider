...
class GoMage_Slider_Adminhtml_ItemsController extends Mage_Adminhtml_Controller_Action{

	
	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu('cms/gomage_slider')
			->_addBreadcrumb(Mage::helper('adminhtml')->__('Slider'), Mage::helper('adminhtml')->__('Slider'));
    ...
    public function saveAction(){
		if ($data = $this->getRequest()->getPost()) {
			try{
				...
				$model = Mage::getModel('gomage_slider/item');					  			
	...
	protected function _deleteItems($ids){
		if(is_array($ids) && !empty($ids)){
			foreach($ids as $id){				
				$item = Mage::getModel('gomage_slider/item')->load($id);
				$item->delete();				
			}
		}
	}
	...
	public function newAction(){
		...
		if($data = Mage::getSingleton('core/session')->getAdspromoData()){
        	Mage::register('gomage_slider', Mage::getModel('gomage_slider/item')->addData($data));
        	Mage::getSingleton('core/session')->setSliderData(null);
        }
		
		$this->_addContent($this->getLayout()->createBlock('gomage_slider/adminhtml_items_edit'))
				->_addLeft($this->getLayout()->createBlock('gomage_slider/adminhtml_items_edit_tabs'));
	...
	public function editAction(){
		...
		if($id = $this->getRequest()->getParam('id')){
        	Mage::register('gomage_slider', Mage::getModel('gomage_slider/item')->load($id));
        }
	...
	public function gridAction()
    {
        $this->getResponse()->setBody(
            $this->getLayout()->createBlock('gomage_slider/adminhtml_items_edit_tab_slider', 'slider.slider.grid')
                ->toHtml()
        );
    }
}